package cognito.amazon.signing.signinamazon.model.network;
...
import cognito.amazon.signing.signinamazon.ui.AuthActivity;
...
public class AuthenticationProvider {
    ...
    public void signOut() {
        Log.d(TAG, "before sign out " + userPool.getCurrentUser().getUserId());
        getCurrentUser().signOut();
        Log.d(TAG, "after sign out: haveCurrentUser() & getCurrentUser().getUserId(): "
    ...
    public void signIn(String login, String password) {
        if (userPool.getUser(login) != null) {
            Log.d(TAG, "<<<provider login>>>  " + login);
            this.login = login;
            this.password = password;
            Log.d(TAG, "<<<provider this.login  >>>  " + this.login);
    ...
    public void getUserAttributes() {
        Log.d(TAG, "<<<get attr user:>>>  " + getCurrentUser().getUserId());
    ...
    private void createCognitoUserPool(Context context) {
        ...
        userPool = new CognitoUserPool(context, USER_POOL_ID, CLIENT_ID, null, Regions.fromName(REGION));
    }
    ...
    private SignUpHandler signUpHandler = new SignUpHandler() {
        ...
        public void onFailure(Exception exception) {
            ...
            if(exception.getMessage().startsWith("User already")) {
                Toast.makeText(context, "You already have a profile. Please log in to your account", Toast.LENGTH_LONG).show();
    ...
    public void resendConfirmationCode(CognitoUser user) {
        user.resendConfirmationCodeInBackground(new VerificationHandler() {
            ...
            public void onSuccess(CognitoUserCodeDeliveryDetails verificationCodeDeliveryMedium) {
                Toast.makeText(context, "We sent you new confirmation code", Toast.LENGTH_SHORT).show();
            }

            @Override
            public void onFailure(Exception exception) {
                Toast.makeText(context, "There was an error with sending new confirmation code", Toast.LENGTH_SHORT).show();
            }
        });
    }
    ...
    private GetDetailsHandler getDetailsHandler = new GetDetailsHandler() {
        ...
        @Override
        public void onFailure(Exception exception) {
            Log.d(TAG, " <<<get attr>>>     " + exception.getMessage());
    ...
    private AuthenticationHandler handler = new AuthenticationHandler() {
        ...
        public void onSuccess(CognitoUserSession userSession, CognitoDevice newDevice) {
            Log.d(TAG, "  <<<auth processs: login & userSession.getUsername()  :   >>>" + login + " " + userSession.getUsername());
        ...
        public void getAuthenticationDetails(final AuthenticationContinuation continuation, final String userID) {
            ...
            Log.d(TAG, "  <<<auth processs: login & userID  :   >>>" + login + " " + userID);
...
//            if(exception.getMessage().startsWith("User is not confirmed")) {
//                final String userID = getCurrentUser().getUserId();
//                if(userID != null) {
//                    resendConfirmationCode(getCurrentUser());
//                } else {
//                    Toast.makeText(context, "Please log in to your account", Toast.LENGTH_SHORT).show();
//                }
//            }
        }
    };
